...
<%@ attribute name="codeName" type="java.lang.String" required="false" description="隐藏域名称（Code）"%>
<%@ attribute name="codeValue" type="java.lang.String" required="false" description="隐藏域值（Code）"%>
...
<%@ attribute name="showFullName" type="java.lang.Boolean" required="false" description="是否显示全名称"%>
<%@ attribute name="disabled" type="java.lang.String" required="false" description="是否限制选择，如果限制，设置为disabled"%>
<%@ attribute name="dataMsgRequired" type="java.lang.String" required="false" description="必填提示信息"%>
<input id="${id}Id" name="${name}" type="hidden" value="${value}"/>
<input id="${id}Code" name="${codeName}" type="hidden" value="${codeValue}"/>
<div class="input-group">
    ...
    <span class="input-group-btn">
         <button type="button" id="${id}Button" class="btn btn-primary
                ...
                ${disabled} ${hideBtn ? 'hide' : ''}">
             <i class="fa fa-search"></i>
         </button>
         <button type="button" id="${id}DelButton" class="close" data-dismiss="alert"
                 style="position: absolute; top: 5px; right: 40px; z-index: 999; display: block;">×</button>
     </span>
</div>
...
function ${id}Layer(text) {
    top.layer.open({
        ...
        content: "${ctx}/tag/treeSelect?url=" + encodeURIComponent("/sys/area/treeData")
        ...
        yes: function (index, layero) {
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                <c:choose>
                <c:when test="${showFullName eq true ? false : true}">
                names.push(nodes[i].text);
                </c:when>
                <c:otherwise>
                $.ajax({
                    url: "${ctx}/sys/area/getFullName?areaId="+nodes[i].id,
                    type: "get", async: false, cache: false,
                    success: function (fullName) { names.push(fullName);}
                });
                </c:otherwise>
                </c:choose>
                <c:if test="${!checked}">
